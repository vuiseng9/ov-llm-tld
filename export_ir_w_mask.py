...
from read_neuralmagic_ref_onnx import ref_mask_from_onnx
...
class Args:
    ...
    quant_mode: str = field(default='original', metadata={
                            "choices": QUANT_MODES})
    ref_sparse_onnx: str = field(
        default='neuralmagic/mpt-7b-gsm8k-pruned70-quant-ds')
...
def apply_mask_from_onnx(args: Args, torch_model: transformers.PreTrainedModel):
    meta = ref_mask_from_onnx(args.ref_sparse_onnx)
    torch_name_2_param = {name: p for name,
                          p in torch_model.named_parameters()}
    for i in range(32):
        name_map = {}
        name_map[f'/transformer/blocks.{i}/attn/Wqkv/module/MatMul.weight_quantized'] = \
            f'transformer.blocks.{i}.attn.Wqkv.weight'
        name_map[f'/transformer/blocks.{i}/attn/out_proj/module/MatMul.weight_quantized'] = \
            f'transformer.blocks.{i}.attn.out_proj.weight'
        name_map[f'/transformer/blocks.{i}/ffn/up_proj/module/MatMul.weight_quantized'] = \
            f'transformer.blocks.{i}.ffn.up_proj.weight'
        name_map[f'/transformer/blocks.{i}/ffn/down_proj/module/MatMul.weight_quantized'] = \
            f'transformer.blocks.{i}.ffn.down_proj.weight'
        for onnx_name, torch_name in name_map.items():
            is_sparse = torch.from_numpy(meta[onnx_name]['is_sparse'])
            is_sparse = is_sparse.t()  # onnx & torch shape transposed
            with torch.no_grad():
                param = torch_name_2_param[torch_name]
                param[is_sparse] = 0.
            with torch.no_grad():
                sparsity = (param == 0).float().mean()
                print(torch_name, 'sparsity=', sparsity)
    return torch_model
...
def main(args: Args):
    ...
    torch_model = apply_mask_from_onnx(args, torch_model)
